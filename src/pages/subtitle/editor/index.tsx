...
import { CompiledASS, DialogueSlice, compile } from "ass-compiler";
...
const Editor = () => {
  const { fansubAcronym, animeSlug, episodeNumber } = useParams<{
  ...
  const updateSlices = useCallback(
    (slices: DialogueSlice[], dialogueIndex: number) => {
  ...
  const updateTime = useCallback(
    (dialogueIndex: number, start: number, end: number) => {
      setSubtitle((currentSubtitle) => {
        if (!currentSubtitle) return currentSubtitle;
    ...
    <div id="subtitle-editor">
      ...
      <div className="video-editor-wrapper">
        ...
        <div
          ...
          style={{ overflowY: subtitle ? "scroll" : "visible" }}
        >
          {subtitle ? (
            <Lines
              subtitle={subtitle}
