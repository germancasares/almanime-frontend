...
const WaveForm = ({
  ...
  useEffect(() => {
    if (!waveSurferRef.current) {
      ...
      waveSurferRef.current = WaveSurfer.create({
        ...
        progressColor:
          initialLocalTheme === Theme.Light ? "#ffc107" : "#005d78",
        plugins: [
          regionsRef.current,
      ...
      regionsRef.current.on("region-updated", ({ id, start, end }) =>
        updateTime(parseInt(id, 10), start, end),
      );

      if (subtitle) {
        getRegionsFromSubtitle(subtitle).forEach((region) =>
          regionsRef.current?.addRegion(region),
        );
      }
    }
  ...
  useMemo(() => {
    ...
    // TODO: Find a better way of using palette
    waveSurferRef.current.setOptions({
      progressColor: localTheme === Theme.Light ? "#ffc107" : "#005d78",
    });
  }, [localTheme]);
