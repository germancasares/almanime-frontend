...
const Menu = ({
  ...
  const onClick = async (event: MouseEvent<HTMLButtonElement>) => {
    ...
    if (!(fansubAcronym && animeSlug && episodeNumber && language && subtitle))
      return;
  ...
  return (
    <div id="menu">
      <button
        type="button"
        className="button"
        onClick={() => setIsStylesActive(true)}
      >
      ...
      {subtitle && (
        ...
        >
          <Icon
            path={
              activeIcon === "mdiDownload" ? mdiDownloadOutline : mdiDownload
            }
            size={1}
          />
        </a>
