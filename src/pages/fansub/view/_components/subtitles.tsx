...
const Subtitles = ({
  ...
  const { data: draftedSubtitles } = FansubApi.GetSubtitlesDrafts(
    acronym,
    accessToken,
  );
  const { data: me } = UserApi.Me(accessToken);
  const canDraft = Helper.HasPermission(Permission.DraftSubtitle, acronym, me);
  const canPublish = Helper.HasPermission(
    Permission.PublishSubtitle,
    acronym,
    me,
  );
  const canUnpublish = Helper.HasPermission(
    Permission.UnpublishSubtitle,
    acronym,
    me,
  );
  const canDelete = Helper.HasPermission(
    Permission.DeleteSubtitle,
    acronym,
    me,
  );
        ...
        {(canPublish || canUnpublish || canDelete) &&
          draftedSubtitles?.map(
            ({
              ...
              <tr key={id}>
                <td>
                  <Link to={routes.anime.view.to(animeSlug)}>{anime}</Link>
                </td>
                <td>{episode}</td>
