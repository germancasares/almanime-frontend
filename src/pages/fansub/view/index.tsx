import { mdiCog, mdiFileUpload, mdiImageText } from "@mdi/js";
...
const CreateSubtitleButton = ({ acronym }: { acronym: string }) => (
  <Link
    ...
    to={routes.subtitle.create.to(acronym)}
  >
    <span className="icon">
...
const UploadSubtitleButton = ({ acronym }: { acronym: string }) => (
  <Link
    className="button is-primary is-rounded"
    to={routes.subtitle.upload.to(acronym)}
  >
    <span className="icon">
...
const EditFansubButton = ({ acronym }: { acronym: string }) => (
  <Link
    className="button is-primary is-rounded"
    to={routes.fansub.edit.to(acronym)}
  >
    <span className="icon">
...
const View = ({ accessToken }: { accessToken?: string }) => {
  ...
  const { data: isMember, isSuccess } = FansubApi.IsMember(
    acronym,
    accessToken,
  );
    ...
    <main id="fansub-view" className="container">
      <section className="section">
        ...
        {activeTab === TabName.Members ? (
          <MembersPage acronym={acronym} />
        ) : null}
        {activeTab === TabName.Subtitles ? (
          <SubtitlesPage acronym={acronym} accessToken={accessToken} />
        ) : null}
      </section>
    </main>
  );
