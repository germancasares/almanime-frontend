...
export const V4PlusStylesSerializer = (styles: V4PlusStyle[]) => {
  const paddings = HEADER_ORDER.reduce(
    (paddingObj: { [header: string]: number }, header) => {
      paddingObj[header] = Math.max(
        header.length,
        ...styles.map(
          (style: { [key: string]: string | number | boolean }) =>
            style[header].toString().length,
        ),
      );
...
Format:`;
  ...
  styleSection += styles
    .map((style) => styleSerializer(style, paddings))
    .join("");
